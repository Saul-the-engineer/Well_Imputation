...
class wellfunc:
    ...
    def extractwelldata(
        self, wells, Left=1948, Right=2018, Min_Obs_Months=50, outlier=3
    ):
        ...
        assert (
            Min_Obs_Months > 0
        ), "Error: Original Data dataframe must have at least 1 example."
        ...
        for col in wells["Data"]:
            wells["Data"][col] = wells["Data"][col][
                np.abs(wells["Data"][col] - wells["Data"][col].mean())
                <= (outlier * wells["Data"][col].std())
            ]
        ...
        centroid.loc["Latitude"][0] = location_df["Latitude"].min() + (
            (location_df["Latitude"].max() - location_df["Latitude"].min()) / 2
        )
        centroid.loc["Longitude"][0] = location_df["Longitude"].min() + (
            (location_df["Longitude"].max() - location_df["Longitude"].min()) / 2
        )
    ...
    def interp_well(self, wells_raw, gap_size="365 days", pad=90, spacing="1MS"):
        ...
        interp_index: DatetimeIndex = pd.date_range(
            start=min(wells_raw.index), freq=spacing, end=max(wells_raw.index)
        )
        ...
        for well in wells_raw:
            ...
            )  # data from last measured point to end
