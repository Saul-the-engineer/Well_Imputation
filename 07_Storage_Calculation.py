...
if units == "Metric":
    unit_coeff, area_coeff, vol_coeff, area_lab, vol_lab = (
        0.3048,
        1,
        1000,
        "km^2",
...
yRes = abs(
    round(imputed_raster["lat"][0] - imputed_raster["lat"][1], 7)
)  # this assumes all cells will be the same size in one dimension (all cells will have same x-component)
...
for y in range(imputed_raster.dimensions["lat"].size):
    ...
    #     radius is 3958.8 mi
    area += (
        (3958.8 * 5280 * unit_coeff) ** 2
        * math.radians(xRes * xCount)
...
drawdown_grid = np.zeros(
    (
        imputed_raster.dimensions["time"].size,
        imputed_raster.dimensions["lat"].size,
        imputed_raster.dimensions["lon"].size,
    )
)
drawdown_volume = np.zeros(imputed_raster.dimensions["time"].size)
for t in range(imputed_raster.dimensions["time"].size):
    drawdown_grid[t, :, :] = (
        imputed_raster["tsvalue"][t, :, :] - imputed_raster["tsvalue"][0, :, :]
    ...
    )  # Average drawdown across entire aquifer * storage_coefficient * area of aquifer

# Plot storage depletion curve
x_data = pd.Series(imputed_raster["time"][:], dtype=int).apply(
    lambda x: datetime.datetime.fromordinal(x)
)  # Convert from days since 0001-01-01 00:00:00
