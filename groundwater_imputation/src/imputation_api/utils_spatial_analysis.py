...
class StorageChangeCalculator:
    ...
    def filter_timeseries(
        ...
        timeseries_start = timeseries.index[0]
        timeseries_end = timeseries.index[-1]
        filter_start = pd.Timestamp(date_range_filter[0])
        filter_end = pd.Timestamp(date_range_filter[1])
        logging.info(f"Available time range: {timeseries_start} - {timeseries_end}")

        if filter_start < timeseries_start:
            filter_start = timeseries_start
            logging.warning(
                f"The start date is not within the time range of the raster. Setting the start date to {filter_start}."
            )
        if filter_end > timeseries_end:
            filter_end = timeseries_end
            logging.warning(
                f"The end date is not within the time range of the raster. Setting the end date to {filter_end}."
            )

        assert filter_start < filter_end, "The start date must be before the end date."
        assert (
            filter_start >= timeseries_start
        ), "The start date must be within the time range of the raster."
        assert (
            filter_end <= timeseries_end
        ), "The end date must be within the time range of the raster."

        logging.info(f"Filtering time range: {filter_start} - {filter_end}")
