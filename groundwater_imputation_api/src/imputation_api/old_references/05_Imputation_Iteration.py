...
for iteration in range(0, iterations):
    ...
    # Measured Well Data
    Well_Data = imp.read_pickle("Well_Data_100", data_root)
    Original_Obs_Points = Well_Data["Data"]
    if iteration == 0:
        Well_Data_Pretrained = imp.read_pickle("Well_Data_Imputed", data_root)
    else:
        Well_Data_Pretrained = imp.read_pickle(
            f"Well_Data_Imputed_iteration_{iteration-1}", data_root
        )
    ...
    Well_Data_Pretrained["Data"] = imp.hampel_filter(
        temp_data, Well_Data["Data"], max_sd=3, window=36
    )
    ...
    Feature_Correlation = pd.DataFrame(
        index=Well_Data["Data"].columns, columns=["FI", "WI"]
    )
    Well_Data["Runs"] = {}
    ...
    for i, well in enumerate(Well_Data["Data"]):
        try:
            ...
            table_dumbies["Months"] = (
                table_dumbies["Months"] / table_dumbies["Months"][-1]
            )
            ...
            pearsons_r = pd.DataFrame(
                pearsons_r.T, index=Feature_Data.columns, columns=["r"]
            )
            ...
            dist = pd.DataFrame(
                cdist(well_loc, feature_loc, metric="euclidean"),
                columns=feature_loc.index,
                index=["dist"],
            ).T
            ...
            fs_data["w_score"] = (
                fs_data["r"] * weight_cor + fs_data["dist"] * weight_dist
            )
            ...
            if feature_thresh - fs_data_score <= 0:
                add_features = 0
            else:
                add_features = int((feature_thresh - fs_data_score) * 10)
            ...
            Feature_Correlation = imp.feature_correlation(
                Feature_Correlation,
                feature_temp,
                Well_Data["Data"],
                fs_data.loc[fs_name],
            ...
            # Create Dataframe to store all predictions from folds and final prediction
            ...
            for train_index, test_index in kfold.split(Y_kfold, X_kfold):
                ...
                x_val = pd.DataFrame(
                    fs.transform(x_val), index=x_val.index, columns=x_val.columns
                )
                x_val = imp.Data_Join(x_val, table_dumbies, method="inner")

                x_test = pd.DataFrame(
                    fs.transform(x_test), index=x_test.index, columns=x_test.columns
                )
                x_test = imp.Data_Join(x_test, table_dumbies, method="inner")

                X_pred_temp = pd.DataFrame(
                    fs.transform(Feature_Data),
                ...
                model.add(Dense(1))
                model.compile(
                    optimizer=opt, loss="mse", metrics=[RootMeanSquaredError()]
                )
                ...
                train_me = (
                    sum(y_train.values - y_train_hat.values) / train_points
                ).item()
                ...
                try:
                    ...
                    test_me = (
                        sum(y_test.values - y_test_hat.values) / test_points
                    ).item()
                    ...
                    temp_metrics.loc[
                        str(j), ["Test ME", "Test RMSE", "Test MAE"]
                    ] = np.NAN
            ...
            Gap_time_series = pd.DataFrame(
                Well_Data["Data"][well], index=Prediction.index
            )
