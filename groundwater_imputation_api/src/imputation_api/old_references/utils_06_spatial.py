...
class krigging_interpolation:
    ...
    def create_grid_polygon(
        self, polygon, x_cells: int = None, y_cells: int = None, res=0.1
    ):
        ...
        if x_cells is not None:
            self.res = float(self.poly_lon_len / x_cells)
        elif y_cells is not None:
            self.res = float(self.poly_lat_len / y_cells)
        ...
        for i, lat in enumerate(grid_lat):
            for j, long in enumerate(grid_long):
                ...
                if not polygon_object.contains(point_temp):
                    mask_array[i, j] = 0
    ...
    def _select_shape(self, polygon):
        ...
        for i, poly in enumerate(polygon):
            if poly["geometry"]["type"] == "Polygon":
                ...
                shapes += list(MultiPolygon(shape(poly["geometry"])))
    ...
    def netcdf_setup(self, grid_long, grid_lat, timestamp, filename):
        ...
        time = file.createDimension(
            "time", None
        )  # time dimension - can extend e.g., size=0
        lat = file.createDimension(
            "lat", lat_len
        )  # create lat dimension in netcdf file of len lat_len
        lon = file.createDimension(
            "lon", lon_len
        )  # create lon dimension in netcdf file of len lon_len

        time = file.createVariable("time", np.float64, ("time"))
        latitude = file.createVariable(
            "lat", np.float64, ("lat")
        )  # create latitude varilable
        longitude = file.createVariable(
            "lon", np.float64, ("lon")
        )  # create longitude varilbe
        tsvalue = file.createVariable(
    ...
    def fit_model_var(
        self, xRange, yRange, values, influence=0.125, bin_num=20, plot=True
    ):
        ...
        binExper, gammaExper = gs.vario_estimate_unstructured(
            (xRange, yRange), values, bins
        )
    ...
    def krig_field(self, var_fitted, x_c, y_c, values, grid_x, grid_y, date, plot=True):
        ...
        krig_map.structured(
            [grid_x, grid_y]
        )  # krig_map.field is the numpy array of values
        ...
        if plot == True:
            ...
            plt.savefig(
                self.figures_root + "/" + str(date.strftime("%Y-%m-%d") + "_01")
            )
            plt.show()
            ...
            plt.savefig(
                self.figures_root + "/" + str(date.strftime("%Y-%m-%d") + "_02")
            )
            plt.show()
        return krig_map
